...
- Release flow: commit the change (with regenerated `dist/` and `resolvers/`), then `npm version patch|minor|major`, then `git push --follow-tags`. The `version` script rebuilds after the bump so generated headers carry the new version, and stages the artifacts into the tag commit.
