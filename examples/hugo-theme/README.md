...
`@sax/design-system`. It shows how a downstream static site consumes the published
...
├── static/
│   ├── css/tokens.css          Vendored from @sax/design-system (see below)
...
committed copies of `@sax/design-system/dist/{tokens,base}.css`. In a real
consumer that installs the package as a git dependency, copy or symlink
them from `node_modules/@sax/design-system/dist/` during deploy rather
