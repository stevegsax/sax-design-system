...
// Concatenated, not @import: the pane injects this file's CONTENT into card
// renders (via the manifest's globalCssPaths), where relative @import URLs
// have nothing to resolve against.
const primitivesCssText = primitivesCss(
  primitiveColor,
  `/* Primitive tonal ramps — specimen cards only. Primitives never appear in\n   product code; reach for semantic or component tokens (see readme). */`,
);
write(
  'styles.css',
  `/* SAX Capital Design System — global entry point. Generated from ${stamp};\n   do not edit here, edit the repo (github.com/stevegsax/sax-design-system).\n   Contains the shipped tokens.css verbatim (base :root + per-situation\n   delta blocks) preceded by specimen-only primitive ramps. */\n${primitivesCssText}\n${tokensCssText}`,
);
write('tokens/primitives.css', primitivesCssText);
...
const CARD_LAYOUT = {
  ...
  'page-shell-application': { situation: 'application', viewport: '1120x800' },
};
...
const rebaseSample = (file, marker) =>
...
write(
  ...
  rebaseSample(
    'marketing.html',
    '<!-- @dsCard group="UI kits" viewport="1120x720" name="Marketing home" subtitle="The shipped marketing sample page, verbatim" -->',
  ),
);
write(
...
write(
  'ui_kits/research-notes/index.html',
  rebaseSample(
    'literary.html',
    '<!-- @dsCard group="UI kits" viewport="900x720" name="Research notes" subtitle="The shipped literary sample page, verbatim" -->',
  ),
);
write(
...
// --- Application prototype fleet (assembled pages as pane cards) --------------------

for (const file of readdirSync(path.join(ROOT, 'prototypes/application'))
  .filter((f) => f.endsWith('.html'))
  .sort()) {
  const fields = patternHeader(readFileSync(path.join(ROOT, 'prototypes/application', file), 'utf8'));
  const name = fields.prototype;
  const title = name[0].toUpperCase() + name.slice(1);
  const src = readFileSync(path.join(ROOT, 'dist/prototypes/application', `${name}.html`), 'utf8')
    .replaceAll('href="../../tokens.css"', 'href="../../styles.css"')
    .replaceAll('../../sax-logo-symbol.svg', '../../static-assets/logos/symbol-only/SAX_logo_symbol.svg');
  write(
    `prototypes/application/${name}.card.html`,
    `<!-- @dsCard group="Application prototypes" viewport="1120x800" name="${title}" subtitle="${fields.archetype ?? ''}" -->\n${src}`,
  );
}
...
- \`patterns/\` — cards for the shipped pattern library (button, field,
  card, alert, tag, callout, page-shell-application), rendered with the
  shipped \`base.css\` element styling under a declared situation.
- \`prototypes/application/\` — the assembled prototype fleet (records,
  settings, dashboard, detail): every page built from the shell pattern
  plus a content module, re-assembled on each sync.
...
// The pane renders from this derived index — card list, token panel, and the
// global CSS it injects into card renders. The app's self-check can rebuild
// it from the @dsCard markers, but it does not re-run reliably after a sync,
// and a stale manifest pointing at deleted files breaks every card. So the
// script owns it, built from the same markers it just wrote.

const dsCardMeta = (html) => {
  const attrs = html.match(/@dsCard\s+([^>]*?)\s*-->/)?.[1] ?? '';
  return Object.fromEntries([...attrs.matchAll(/(\w+)="([^"]*)"/g)].map(([, k, v]) => [k, v]));
};

const cardFiles = [];
const walkOut = (dir, rel = '') => {
  for (const entry of readdirSync(path.join(OUT, dir))) {
    const abs = path.join(OUT, dir, entry);
    const relPath = rel ? `${rel}/${entry}` : entry;
    if (statSync(abs).isDirectory()) walkOut(path.join(dir, entry), relPath);
    else if (relPath.endsWith('.card.html') || /^ui_kits\/.+\/index\.html$/.test(relPath)) {
      cardFiles.push(relPath);
    }
  }
};
walkOut('.');

const tokenKind = (name) => {
  if (name.includes('radius')) return 'radius';
  if (/--(space|rhythm|container|border-width)|padding|width/.test(name)) return 'spacing';
  if (/typography|font/.test(name)) return 'font';
  return 'color';
};
const cssDecls = (css) =>
  [...css.replace(/\/\*[\s\S]*?\*\//g, '').matchAll(/(--[a-z0-9-]+):\s*([^;]+);/g)].map(([, name, value]) => ({
    name,
    value: value.trim(),
  }));
...
const manifest = {
  namespace: GLOBAL,
  components: COMPONENTS.map((c) => ({ name: c.name, sourcePath: `${c.dir}/${c.name}.jsx` })),
  startingPoints: [],
  cards: cardFiles
    .sort()
    .map((p) => {
      const m = dsCardMeta(readFileSync(path.join(OUT, p), 'utf8'));
      return { path: p, group: m.group, viewport: m.viewport, subtitle: m.subtitle, name: m.name };
    })
    .filter((c) => c.group),
  templates: [
    {
      ...
      description:
        'SAX Capital marketing home — masthead, hero, feature cards, stat band, access form, and footer, from design-system tokens.',
      folder: 'templates/marketing-page',
      entryPath: 'templates/marketing-page/MarketingPage.dc.html',
      thumbnail: { path: 'templates/marketing-page/.thumbnail', kind: 'captured' },
    },
    {
      name: 'Research Note',
      description:
        'SAX Capital research blog post — masthead, title, byline, topic tags, and article body with blockquote and code, from design-system tokens.',
      folder: 'templates/research-note',
      entryPath: 'templates/research-note/ResearchNote.dc.html',
      thumbnail: { path: 'templates/research-note/.thumbnail', kind: 'captured' },
    },
  ],
  hasThumbnailHtml: false,
  globalCssPaths: ['styles.css'],
  tokens: [
    ...cssDecls(primitivesCssText).map(({ name, value }) => ({
      name,
      value,
      kind: 'color',
      definedIn: 'tokens/primitives.css',
    })),
    ...rootVars(tokensCssText).map(({ name, value }) => ({
      name,
      value,
      kind: tokenKind(name),
...
write('_ds_manifest.json', `${JSON.stringify(manifest)}\n`);
