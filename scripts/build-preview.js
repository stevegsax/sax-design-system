...
const CHROME = `
  * { box-sizing: border-box; }
  body { margin: 0 auto; max-width: 72rem; padding: var(--space-xl);
         background: var(--color-background-page); color: var(--color-text-body);
  ...
  .bar { height: 1rem; background: var(--color-accent-default); border-radius: var(--radius-sm); }
...
for (const product of matrix.products) {
  const [light, dark] = await Promise.all(['light', 'dark'].map((m) => cssTokensFor(product, m)));
...
<title>${pkg.name} — ${product} tokens</title>
<link rel="stylesheet" href="tokens.css">
<style>${CHROME}</style>
</head>
<body>
<h1>${product} design tokens</h1>
<p>Generated by <code>${pkg.name}</code> v${pkg.version}. Light and dark render side by side via
<code>color-scheme</code>; every sample below is driven by the custom properties in <code>tokens.css</code>.</p>
${rampSection(light)}
<section>
  <h2>Color roles</h2>
  <div class="panels">
...
`;
  ...
  console.log(`dist/${product}/preview.html`);
}
